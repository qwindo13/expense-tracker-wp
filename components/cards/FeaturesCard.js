...
const FeaturesCard = ({ title, description, icon, image, highlight }) => {
    return (
        ...
        <h3 class={`text-2xl 2xl:text-3xl font-semibold ${highlight ? 'text-white' : 'text-[#2c2c2c]'} my-4`}>{title}</h3>
        {description && <p class="text-base 2xl:text-lg text-[#2c2c2c] opacity-50 font-medium">{description}</p>}
